...
impl Downloader for S3Downloader {
    async fn download(&self, file_id: String, start: u64, length: usize) -> Result<Vec<u8>> {
        ...
        println!("download {} {}", file_id, &range);
        ...
        let obj: GetObjectOutput = self.client.get_object(get_obj_req).await?;
        let mut reader = obj.body.unwrap().into_async_read();
        let mut res = vec![];
        res.reserve(length);
        let bytes_read = reader.read_to_end(&mut res).await?;
        if bytes_read != length {
            bail!("Not the expected number of bytes".to_owned(),)
        } else {
            Ok(res)
        }
    }
...
pub fn downloader_creator(region: &str) -> (String, Box<dyn Fn() -> Arc<dyn Downloader>>) {
