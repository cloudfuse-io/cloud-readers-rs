...
impl FileCache {
    ...
    pub(crate) fn insert(&self, start: u64, download: Download) {
...
///
/// To get a [`FileManager`], you need to register a [`FileDescription`](super::FileDescription) on a [`DownloadCache`](super::DownloadCache).
...
impl FileManager {
    pub(crate) fn new(cache: FileCache, tx: UnboundedSender<Range>) -> Self {
        Self { cache, tx }
    }
    ...
    pub(crate) fn get_range(&self, start: u64) -> Result<RangeCursor> {
